...
structure parserMotion : PARSERMOTION =
  ...
  *)
  fun fixOrigin conn =
    case conn of
      Construction.Source(token, typeName) => if String.substring(typeName, 0, 1) = "o" then  Construction.Source("o"^token, "origin") else  Construction.Source(token, typeName)
  ...
  fun parseRule p branch depth stri =
      ...
      if exists str #"V" then Construction.TCPair({constructor = ("logicApplyQuant", (["forall", "point", "formula"], "formula")), token =("tqnt"^p^d, "formula")},
                          ...
                          print(right^"\n");Construction.TCPair({constructor = ("prefixBinRel", (["binRel", "point", "point"], "formula")), token =("tfrt"^p^d, "formula")}, [Construction.Source ("fr"^p^d, "further"), Construction.Source("p"^p^"1"^d, String.substring (right, 0,2)^":point"), Construction.Source("p"^p^"2"^d, String.substring (right, 2,2)^":point")]) end 
      else if exists str #"C" then 
                          let val right = SafterSepS #"C" stri in
                          print(right^"\n");Construction.TCPair({constructor = ("prefixBinRel", (["binRel", "point", "point"], "formula")), token =("tclr"^p^d, "formula")},[Construction.Source ("cl"^p^d, "closer"), Construction.Source("p"^p^"1"^d, String.substring (right, 0,2)^":point"), Construction.Source("p"^p^"2"^d, String.substring (right, 2,2)^":point")]) (*issue here*)  end 
