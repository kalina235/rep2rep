...
∀ x1-> V x1 | > -> implies | * -> <=> | A -> and between rules | & -> and within the rules | ~ -> not | closer -> S | further -> L | away -> a | towards -> T
  ...
  *)
  ...
  fun parseRule p branch depth stri =
      ...
      if exists str #"V" then Construction.TCPair({constructor = ("logicApplyQuant", (["forall", "point", "formula"], "formula")), token =("tqnt"^p^d, "formula")},
                          ...
                          print(right^"\n");Construction.TCPair({constructor = ("prefixTerRel", (["prefixTerRel", "point", "point", "point"], "formula")), token =("ttow"^p^d, "formula")}, [Construction.Source ("tow"^p^d, "towards"), Construction.Source("p"^p^"1"^d, String.substring (right, 0,2)^":point"), Construction.Source("p"^p^"2"^d, String.substring (right, 2,2)^":point"), Construction.Source("p"^p^"3"^d, String.substring (right, 4,2)^":point")]) end
